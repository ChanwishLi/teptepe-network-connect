...
export function OpportunityPostForm() {
  ...
  const submit = useMutation({
    mutationFn: async () => {
      if (!user) throw new Error("Please log in before posting an opportunity.");
      if (!form.position.trim() || !form.company_name.trim() || !form.description.trim()) {
    ...
    onError: (e: any) => toast.error(e.message || "Could not submit opportunity."),
  });
    ...
    <PageShell>
      <section className="mx-auto max-w-2xl px-4 py-12 lg:px-8">
        ...
        <Card className="mt-8 space-y-4 p-6">
          ...
          <Row label="Description *"><Textarea rows={6} value={form.description} onChange={(e) => setForm({ ...form, description: e.target.value })} /></Row>
          ...
          <Row label="Contact email"><Input type="email" value={form.contact_email} onChange={(e) => setForm({ ...form, contact_email: e.target.value })} placeholder={user.email ?? "contact@example.com"} /></Row>
          <Row label="Application deadline"><Input type="date" value={form.deadline} onChange={(e) => setForm({ ...form, deadline: e.target.value })} /></Row>
